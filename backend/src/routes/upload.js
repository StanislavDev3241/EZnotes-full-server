...
const processFileWithOpenAI = async (
  ...
  try {
    ...
    const shouldGenerateSoap =
      selectedNoteTypes.includes("soap") &&
      (!customPrompt?.noteType ||
        customPrompt.noteType === "soap" ||
        customPrompt.noteType === "both");

    const shouldGenerateSummary =
      selectedNoteTypes.includes("summary") &&
      (!customPrompt?.noteType ||
        customPrompt.noteType === "summary" ||
        customPrompt.noteType === "both");
      ...
      try {
        const patientSummaryResult = await openaiService.generatePatientSummary(
          transcription,
          {
            userId: userId,
            procedureType: "general",
            customPrompt: customPrompt, // ✅ NEW: Pass custom prompt to patient summary generation
          }
        );
    ...
    if (notes.soapNote) {
      ...
      // ✅ REMOVED: Conversation creation moved to when user clicks "Continue Chat"
      // This ensures conversation is only created when user actually wants to chat
    }
    ...
    if (notes.patientSummary) {
      ...
      // ✅ REMOVED: Patient summary message creation moved to when user clicks "Continue Chat"
    }
    ...
    try {
      ...
      if (req.body.customPromptId) {
        ...
        if (savedPrompt && savedPrompt.user_id === userId) {
          ...
          console.warn(
            `⚠️ Custom prompt not found or access denied: ${req.body.customPromptId}`
          );
        }
