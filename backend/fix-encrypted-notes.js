    ...
    for (const note of result.rows) {
      try {
        ...
        try {
          ...
          console.log(`❌ Note ${note.id} fails to decrypt: ${decryptError.message}`);
        }
        ...
        if (note.file_id) {
          ...
          if (notesResult.rows.length > 0) {
            ...
            if (filesResult.rows.length > 0 && filesResult.rows[0].transcription) {
              fallbackContent = filesResult.rows[0].transcription;
              console.log(`📄 Found content in files table for note ${note.id}`);
            }
          }
        }
        ...
        if (fallbackContent) {
          ...
          const reEncrypted = encryptionUtils.encryptData(fallbackContent, note.user_id);
          ...
          console.log(`✅ Fixed note ${note.id} - re-encrypted with current key`);
          fixedCount++;
        } else {
