-- Encrypted Tables Migration for HIPAA Compliance
-- This adds encrypted storage for sensitive data

-- Table for encrypted saved notes
CREATE TABLE IF NOT EXISTS encrypted_saved_notes (
  id SERIAL PRIMARY KEY,
  user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  note_type VARCHAR(50) NOT NULL, -- 'soap_note', 'patient_summary', 'custom'
...
CREATE TABLE IF NOT EXISTS audit_logs (
  ...
  action_type VARCHAR(100) NOT NULL, -- 'login', 'data_access', 'data_modify', 'encrypt', 'decrypt'
