...
const ManagementPage: React.FC<ManagementPageProps> = ({ 
  ...
  const loadData = async () => {
    ...
    try {
      ...
      switch (activeTab) {
        case "chat":
          const chatResponse = await fetch(`${API_BASE_URL}/api/chat/history/${user.id}`, {
        ...
        case "notes":
          const notesResponse = await fetch(`${API_BASE_URL}/api/notes/user/${user.id}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          if (notesResponse.ok) {
        ...
        case "files":
          const filesResponse = await fetch(`${API_BASE_URL}/api/files/user/${user.id}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          if (filesResponse.ok) {
  ...
  const filteredFiles = files.filter(file =>
    ...
    (file.transcription && file.transcription.toLowerCase().includes(searchTerm.toLowerCase()))
  );
              ...
              <div className="space-y-4">
                ...
                {filteredConversations.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-sm">Start chatting with AI to see your conversation history</p>
                  </div>
                ) : (
                      ...
                      <div key={conversation.id} className="bg-white rounded-lg shadow-sm border p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600">File: {conversation.filename}</p>
              ...
              <div className="space-y-4">
                ...
                {filteredNotes.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-sm">Upload files to generate AI-powered medical notes</p>
                  </div>
                ) : (
                  <div className="grid gap-4">
                      ...
                      <div key={note.id} className="bg-white rounded-lg shadow-sm border p-4">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            note.status === 'generated' ? 'bg-green-100 text-green-800' :
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500">
                          <span>Prompt: {note.prompt_used.substring(0, 50)}...</span>
              ...
              <div className="space-y-4">
                ...
                {filteredFiles.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-sm">Upload audio or text files to get started</p>
                  </div>
                ) : (
                  <div className="grid gap-4">
                      ...
                      <div key={file.id} className="bg-white rounded-lg shadow-sm border p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-2">
                            {file.transcription && (
                              <button
                                onClick={() => downloadTranscription(file.transcription, file.original_name)}
                        ...
                        {file.transcription && (
                          <div className="mt-3 p-3 bg-gray-50 rounded-md">
                            <p className="text-xs text-gray-600 mb-1">Transcription Preview:</p>
